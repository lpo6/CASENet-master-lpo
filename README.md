...
本文为论文《CASENet：深度类别感知语义边缘检测》的PyTorch实现。

本代码库已适配适用于Cityscapes数据集。

## 输入数据预处理
使用了作者提供的预处理代码库，具体使用说明可在`cityscapes-preprocess`目录中找到。该工具用于为数据集中的每个标签生成二进制文件。

如需将数据加载到hdf5文件中，需要生成包含这些二进制文件的hdf5文件。进行此转换时，请运行：
...
使用的模型是CASENet的ResNet-101变体。

模型配置文件(.prototxt)可在：[此处]获取。

CASENet预训练权重的下载链接可在：[此处]获取。

## 将Caffe权重转换为PyTorch格式
要在PyTorch中使用预训练的caffemodel，请使用`extract-caffe-params`将各层权重保存为numpy格式。相关代码及使用说明可在`utils`文件夹中找到。

要将这些各层的numpy权重加载到PyTorch模型中，请运行：

```bash
python modules/CASENet.py
```
（运行前请将[此行]的目录路径更新为包含numpy文件的父目录绝对路径）

注意：PyTorch预训练权重可从[Google网盘]下载。
...
```
-h, --help              显示帮助信息并退出
--checkpoint-folder     检查点目录路径（默认：./checkpoint）
--multigpu              使用多GPU（默认：False）
-j, --workers           数据加载工作进程数（默认：16）
--epochs                总训练轮数（默认：150）
--start-epoch           手动设置起始轮数（适用于重启训练）
--cls-num               类别数量（Cityscapes数据集默认为19）
--lr-steps              学习率衰减10倍的迭代次数
--acc-steps             梯度累积步数（默认：1）
-b, --batch-size        迷你批次大小（默认：1）
--lr                    学习率（默认：1e-7）
--momentum              动量（默认：0.9）
--weight-decay          权重衰减（默认：1e-4）
-p, --print-freq        打印频率（默认：10）
--resume-model          最新检查点路径（默认：None）
--pretrained-model      预训练检查点路径（默认：None）
```

## 可视化
要可视化特征图、真实标签和预测结果，请运行：

```bash
python visualize_multilabel.py [--model 模型路径] [--image_file 图像文件] [--image_dir 图像目录] [--output_dir 输出目录]
```

例如，在Cityscapes数据集的验证集上进行可视化：
...
```bash
python get_results_for_benchmark.py [--model 模型路径] [--image_file 图像文件] [--image_dir 图像目录] [--output_dir 输出目录]
```
...
## 额外改进
- 添加了将边缘转换为轮廓并进行可视化的脚本
- 实现了梯度累积，可在不增加内存使用的情况下增大批次大小
- 清空未使用变量以释放缓存内存

## 致谢
数据处理基于以下代码库：https://github.com/Chrisding/cityscapes-preprocess

caffemodel到numpy格式的转换使用以下代码库：https://github.com/nilboy/extract-caffe-params

模型主要参考以下语义边界数据集的代码库：https://github.com/lijiaman/CASENet

论文的原始Caffe实现可从以下链接下载：http://www.merl.com/research/license#CASENet
