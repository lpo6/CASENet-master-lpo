...
def train(args, train_loader, model, optimizer, epoch, curr_lr, win_feats5, win_fusion, viz, global_step,
    ...
    for i, (img, target) in enumerate(train_loader):
        ...
        print(f"🔄 批次 {i} 开始 - 内存: {start_memory:.2f}GB")
        ...
        # 🚨 监控前向传播前
        pre_forward_memory = torch.cuda.memory_allocated() / 1024 ** 3 if torch.cuda.is_available() else 0
        print(f"  📈 前向传播前 - 内存: {pre_forward_memory:.2f}GB")

        score_feats5, fused_feats = model(img_var)  # BS X NUM_CLASSES X 472 X 472

        # 🚨 监控前向传播后
        post_forward_memory = torch.cuda.memory_allocated() / 1024 ** 3 if torch.cuda.is_available() else 0
        print(f"  📈 前向传播后 - 内存: {post_forward_memory:.2f}GB")
        ...
        # 🚨 监控损失计算后
        post_loss_memory = torch.cuda.memory_allocated() / 1024 ** 3 if torch.cuda.is_available() else 0
        print(f"  📈 损失计算后 - 内存: {post_loss_memory:.2f}GB")

        loss.backward()

        # 🚨 监控反向传播后
        post_backward_memory = torch.cuda.memory_allocated() / 1024 ** 3 if torch.cuda.is_available() else 0
        print(f"  📈 反向传播后 - 内存: {post_backward_memory:.2f}GB")
        ...
        # 🚨 监控清理后
        post_clean_memory = torch.cuda.memory_allocated() / 1024 ** 3 if torch.cuda.is_available() else 0
        print(f"  🧹 清理后 - 内存: {post_clean_memory:.2f}GB")
        ...
        if (i + 1) % accumulation_steps == 0:
            ...
            # Only plot the fused feats loss.
            trn_feats5_loss = feats5_loss.detach().clone().cpu().data.numpy()
            trn_fusion_loss = fused_feats_loss.detach().clone().cpu().data.numpy()
            viz.line(win=win_feats5, name='train_feats5', update='append', X=np.array([global_step]),
                     Y=np.array([trn_feats5_loss]))
            viz.line(win=win_fusion, name='train_fusion', update='append', X=np.array([global_step]),
            ...
            # 🚨 在优化器步骤后释放损失变量
            del feats5_loss, fused_feats_loss, loss
            torch.cuda.empty_cache()

            # 🚨 监控优化器步骤后
            post_optim_memory = torch.cuda.memory_allocated() / 1024 ** 3 if torch.cuda.is_available() else 0
            print(f"  🔧 优化器步骤后 - 内存: {post_optim_memory:.2f}GB")
...
def WeightedMultiLabelSigmoidLoss(model_output, target):
    ...
    print(f"    📊 损失函数开始 - 内存: {start_mem:.2f}GB")

    # Calculate weight. (edge pixel and non-edge pixel)
    with torch.no_grad():
        weight_sum = utils.check_gpu(0, target.sum(dim=1).sum(dim=1).sum(dim=1).float().data)
        edge_weight = utils.check_gpu(0, weight_sum.data / float(target.size()[1] * target.size()[2]))
    ...
    target_transposed = target.transpose(1, 3).transpose(2, 3).float()

    # 计算损失
    loss = -non_edge_weight.unsqueeze(1).unsqueeze(2).unsqueeze(3) * target_transposed * torch.log(
        one_sigmoid_out.clamp(min=1e-10)) - \
           edge_weight.unsqueeze(1).unsqueeze(2).unsqueeze(3) * (1 - target_transposed) * torch.log(
        (1 - one_sigmoid_out).clamp(min=1e-10))

    result = loss.mean(dim=0).sum()

    # 🚨 监控损失函数结束
    end_mem = torch.cuda.memory_allocated() / 1024 ** 3 if torch.cuda.is_available() else 0
    print(f"    📊 损失函数结束 - 内存: {end_mem:.2f}GB, 增量: {end_mem - start_mem:.2f}GB")
